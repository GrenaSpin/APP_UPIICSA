...
const presentationTitleInput = $("#presentation-title");
...
let state = {
  ...
  selectedElId: null,
  dirty: false,
  title: presentationTitleInput?.value || "Presentación",
};
...
function defaultSlide(){
  return { id: uid("slide"), title: `Slide ${state.slides.length+1}`, bg:"#ffffff", elements:[] };
}
function defaultTextEl(){
  return { id: uid("el"), type:"text", x:80,y:80,w:260,h:70, color:"#111111", text:"Doble click para editar" };
}
function defaultRectEl(){
  return { id: uid("el"), type:"rect", x:120,y:140,w:260,h:160, color:"#6ea8fe", text:"" };
}
function defaultImageEl(src){
  return { id: uid("el"), type:"image", x:140,y:90,w:320,h:200, color:"#000000", text:"", src };
}

function activeSlide(){ return state.slides.find(s => s.id === state.activeSlideId) || null; }
function activeEl(){
  const s = activeSlide(); if(!s) return null;
...
async function api(action, payload){
  const res = await fetch("api.php?action=" + encodeURIComponent(action), {
    ...
    body: JSON.stringify({ csrf: window.CSRF_TOKEN, presentation_id: window.PRESENTATION_ID, ...payload })
  });
...
btnAddText.addEventListener("click", () => {
  ...
  renderSlidesList(); renderSlide(); markDirty(true);
});
...
imagePicker.addEventListener("change", async () => {
  ...
  try{
    ...
    fd.append("presentation_id", String(window.PRESENTATION_ID));
...
presentationTitleInput.addEventListener("input", () => {
  state.title = presentationTitleInput.value.trim() || "Presentación";
  markDirty(true);
});
...
btnSave.addEventListener("click", async () => {
  try{
    setStatus("Guardando...");
    const payload = { title: state.title, content: { slides: state.slides } };
    const r = await api("save_presentation", payload);
...
function startDrag(ev, node){
  ...
  function onMove(e){
    ...
    if(state.selectedElId === el.id){
      propX.value = el.x; propY.value = el.y;
    }
  }
  function onUp(){
...
function startResize(ev, node, handle){
  ...
  function onMove(e){
    ...
    if(hasE) nw = o.w + dx;
    if(hasS) nh = o.h + dy;
    if(hasW){ nw = o.w - dx; nx = o.x + dx; }
    if(hasN){ nh = o.h - dy; ny = o.y + dy; }
...
function enableInlineEdit(node){
  ...
  function finish(save=true){
    ...
    if(save) el.text = node.textContent || "";
...
async function boot(){
  setStatus("Cargando...");
  const r = await api("get_presentation", {});
  if(!r.ok){ setStatus("Error: " + (r.error||"")); return; }

  state.title = r.title || state.title;
  presentationTitleInput.value = state.title;

  const content = r.content || {};
  state.slides = Array.isArray(content.slides) ? content.slides : [defaultSlide()];
