...
/* =======================
   SINCRONIZAR SELECT "IR A SLIDE"
   ======================= */
function syncSlideJump() {
    const sel = document.getElementById("slide-jump");
    if (!sel) return;

    sel.innerHTML = slides.map((s, i) => {
        const t = (s.titulo || `Slide ${i+1}`).replace(/</g, "&lt;").replace(/>/g, "&gt;");
        return `<option value="${i}">Slide ${i + 1}: ${t}</option>`;
    }).join("");

    sel.value = String(Math.max(actual, 0));
}
